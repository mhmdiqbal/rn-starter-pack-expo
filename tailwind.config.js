// tailwind.config.js
const defaultTheme = require("tailwindcss/defaultTheme");

/** @type {import('tailwindcss').Config} */
module.exports = {
  content: ["./app/**/*.{js,jsx,ts,tsx}", "./components/**/*.{js,jsx,ts,tsx}"],
  darkMode: "class",
  theme: {
    extend: {
      colors: {
        // Primary colors
        primary: {
          100: "#F4E3E2",
          200: "#CB9593",
          300: "#A65C5A",
          400: "#862C2C",
          500: "#571916",
          600: "#330A08",
          700: "#220402",
        },

        // Linear colors
        linear: {
          1: "#F4E3E2",
          2: "#CB9593",
        },

        // Pressed colors
        pressed: {
          1: "rgba(244, 227, 226, 0.75)",
          2: "rgba(203, 149, 147, 0.75)",
          3: "rgba(87, 25, 22, 0.75)",
          4: "rgba(216, 220, 223, 0.75)",
          5: "rgba(141, 9, 9, 0.75)",
          6: "rgba(246, 222, 221, 0.75)",
          7: "rgba(223, 234, 255, 0.75)",
          8: "rgba(16, 65, 160, 0.75)",
          9: "rgba(8, 112, 37, 0.75)",
        },

        // Supplementary colors
        sup: {
          yellow: "#FAB8B8",
          red: "#FEF5E2",
        },

        // Success colors
        success: {
          100: "#DFEAFF",
          200: "#B0C7F5",
          300: "#5D8CE4",
          400: "#2B5FC3",
          500: "#1041A0",
          600: "#012873",
          700: "#001C51",
        },

        // Positive colors
        positive: {
          100: "#E4FAEA",
          200: "#88D49E",
          300: "#3AB45C",
          400: "#179139",
          500: "#087025",
          600: "#034415",
          700: "#01260B",
        },

        // Error colors
        error: {
          100: "#F6DEDD",
          200: "#FABBBB",
          300: "#DF6464",
          400: "#B52626",
          500: "#8D0909",
          600: "#570101",
          700: "#340000",
        },

        // Warning colors
        warning: {
          100: "#FCF4E2",
          200: "#F6E0A2",
          300: "#E1BA54",
          400: "#CE9E22",
          500: "#AC7D05",
          600: "#795701",
          700: "#453200",
        },

        // Neutral colors
        neutral: {
          100: "#FBFBFB",
          200: "#F5F5F5",
          300: "#D8DCDF",
          400: "#989B9E",
          500: "#5D5F60",
          600: "#3D3D3D",
          700: "#0A0A0A",
        },

        // Transparent
        transparent: "transparent",
      },
      fontFamily: {
        sans: ["Regular"],
        'sans-medium': ["Medium"],
        'sans-semi-bold': ["SemiBold"],
        'sans-bold': ["Bold"],
      },
      fontSize: {
        // Headings
        "heading-1": [
          "3rem",
          { lineHeight: "3.875rem", letterSpacing: "-0.01em" },
        ],
        "heading-2": [
          "2.25rem",
          { lineHeight: "3rem", letterSpacing: "-0.01em" },
        ],
        "heading-3": ["1.5rem", { lineHeight: "2.25rem" }],
        "heading-4": ["1.25rem", { lineHeight: "1.75rem" }],
        "heading-5": ["1rem", { lineHeight: "1.5rem" }],

        // Body Text
        "body-xl": ["1.25rem", { lineHeight: "1.75rem" }],
        "body-lg": ["1rem", { lineHeight: "1.5rem" }],
        "body-md": ["0.875rem", { lineHeight: "1.25rem" }],
        "body-sm": ["0.75rem", { lineHeight: "1.125rem" }],
        "body-xs": [
          "0.625rem",
          { lineHeight: "1rem", letterSpacing: "0.01em" },
        ],

        // Buttons
        "button-lg": ["1.25rem", { lineHeight: "1.75rem", fontWeight: "600" }],
        "button-md": ["1rem", { lineHeight: "1.5rem", fontWeight: "600" }],
        "button-sm": ["0.75rem", { lineHeight: "1.125rem", fontWeight: "600" }],

        // Labels
        "label-xl": ["1.25rem", { lineHeight: "1.75rem", fontWeight: "500" }],
        "label-lg": ["1rem", { lineHeight: "1.5rem", fontWeight: "500" }],
        "label-md": ["0.875rem", { lineHeight: "1.25rem", fontWeight: "500" }],
        "label-sm": ["0.75rem", { lineHeight: "1.125rem", fontWeight: "500" }],
        "label-xs": [
          "0.625rem",
          {
            lineHeight: "0.875rem",
            fontWeight: "500",
            letterSpacing: "0.01em",
          },
        ],
      },
      fontWeight: {
        normal: "400",
        medium: "500",
        "semi-bold": "600",
      },
    },
  },
};
